...
public abstract class BmX280TemperatureSensor : ITemperatureSensor, IDisposable
{
  private const int DEVICE_OPEN_RETRIES = 5;

  private readonly ILogger _logger;
  private readonly I2CSensorConfiguration _sensorConfiguration;
  public string Name => $"{SensorTypeName}-Bus-{_sensorConfiguration.BusId}-Addr-{_i2cAddress}";

  private readonly int _i2cAddress;
  private I2cDevice? _i2cDevice;
  private Bmx280Base? _sensor;

  protected abstract string SensorTypeName { get; }
  protected abstract Func<I2cDevice, Bmx280Base> SensorFactory { get; }

  protected BmX280TemperatureSensor(
    ILogger logger,
  ...
  )
  {
    ...
    _i2cAddress = _sensorConfiguration.I2CAddress ?? Bmx280Base.DefaultI2cAddress;
  ...
  private void CreateSensor(I2CSensorConfiguration sensorConfiguration)
  {
    ...
    for (; i <= DEVICE_OPEN_RETRIES; i++)
    {
      ...
      {
        _logger.LogDebug(
          "Starting temperature sensor {SensorName} on bus {busId} and address {addr}.",
          SensorTypeName,
        ...
        _sensor = SensorFactory(_i2cDevice);
